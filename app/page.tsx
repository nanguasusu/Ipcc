...
import { calculateScore, calculateStats, latencyLabel, medianProbeResult, runHttpProbe, runWithConcurrency, targets, type ProbeResult, type TestStatus } from './lib/network';
...
function chartTone(sample: ProbeResult) { if (!sample.success || sample.latency === null || sample.latency >= 500) return 'bad'; return sample.latency >= 150 ? 'warn' : 'good'; }
function formatCountry(country: string | null) { return country ? countryNames.of(country) ?? country : '等待实时数据'; }
...
export default function Home() {
  ...
  const runAll = async () => {
    ...
    const samples = new Map<string, ProbeResult[]>();
    try {
      await runWithConcurrency(targets, 3, (target) => runHttpProbe(target, { signal: active.signal }));
      for (let sample = 0; sample < 3; sample += 1) {
        const wave = await runWithConcurrency(targets, 3, (target) => runHttpProbe(target, { signal: active.signal }));
        for (const result of wave) {
          const targetSamples = samples.get(result.targetId) ?? [];
          targetSamples.push(result);
          samples.set(result.targetId, targetSamples);
          updateResult(medianProbeResult(targetSamples));
        }
      }
    }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- 页面只在初次进入时自动开始一次完整检测。
  useEffect(() => { const timer = window.setTimeout(() => void runAll(), 0); return () => window.clearTimeout(timer); }, []);
    ...
    <div className="window-frame">
      ...
      <div className="app-layout">
        ...
        <div className="content-area">
          <section className="hero" id="overview"><div><p className="eyebrow">浏览器网络诊断</p><h1>看见你的网络，<em>正在发生什么。</em></h1><p className="hero-copy">从当前浏览器出发，测量真实的访问路径、延迟与稳定性。所有结果只保留在你的设备上。</p></div>{(isRunning || stabilityRunning) && <div className="hero-actions"><button className="button secondary" onClick={stopAll}>停止检测</button></div>}</section>

          <section className="overview-grid" aria-label="网络概览">
            <article className="card ip-card pastel-blue"><div className="card-heading"><div><p className="eyebrow">当前公网 IP</p><h2>{ipLoading ? '获取中…' : ipInfo?.ip ?? '暂不可用'}</h2></div><div className="flag-orbit"><Flag country={ipInfo?.country ?? null} /></div></div>{ipError ? <p className="error-copy">{ipError}</p> : <div className="info-list"><span><small>位置</small><b>{formatCountry(ipInfo?.country ?? null)}</b></span><span><small>网络</small><b>{ipInfo?.asn ?? '—'} {ipInfo?.organization ?? ''}</b></span><span><small>类型</small><b>{ipInfo?.networkType ?? '未知'} {ipInfo?.suspectedProxy ? '· 可能使用代理' : ''}</b></span></div>}<div className="card-footer"><button className="text-button" onClick={() => navigator.clipboard.writeText(ipInfo?.ip ?? '')} disabled={!ipInfo?.ip}>复制 IP</button><button className="text-button" onClick={() => void fetchIp()}>刷新</button></div></article>
          ...
          <section className="stability-grid"><article className="card chart-card"><div className="chart-title"><div><h3>实时响应曲线</h3><p>Cloudflare、GitHub、ChatGPT、Gemini、YouTube</p></div><span className={stabilityRunning ? 'live-pill active' : 'live-pill'}>{stabilityRunning ? '● 实时' : '已暂停'}</span></div><div className="chart" aria-label="延迟采样图">{stabilitySamples.length ? stabilitySamples.slice(-45).map((sample, index) => <span key={`${sample.targetId}-${sample.timestamp}`} title={`${sample.targetId}: ${formatLatency(sample.latency)}`} className={`chart-bar chart-${chartTone(sample)}`} style={{ height: `${sample.latency ? Math.min(100, Math.max(12, 100 - sample.latency / 8)) : 8}%`, animationDelay: `${index * 20}ms` }} />) : <p>开始稳定性检测，在这里查看实时响应曲线。</p>}</div><p className="chart-caption">绿色：&lt;150 ms；黄色：150–499 ms；红色：≥500 ms、超时或失败。</p></article><article className="card stats-card pastel-lilac"><p className="eyebrow">请求质量</p><div className="stat-matrix"><span><small>采样数</small><b>{stabilityStats.samples}</b></span><span><small>成功率</small><b>{stabilityStats.samples ? `${stabilityStats.successRate}%` : '—'}</b></span><span><small>平均延迟</small><b>{formatLatency(stabilityStats.average)}</b></span><span><small>P95</small><b>{formatLatency(stabilityStats.p95)}</b></span><span><small>抖动</small><b>{formatLatency(stabilityStats.jitter)}</b></span><span><small>请求丢失</small><b>{stabilityStats.samples ? `${stabilityStats.requestLoss}%` : '—'}</b></span></div><p className="note">抖动仅比较同一服务的相邻采样；请求丢失不等同于网络丢包。</p></article></section>
